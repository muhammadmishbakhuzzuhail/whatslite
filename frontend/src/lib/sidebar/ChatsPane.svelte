<script>
  ...
  import { syncing, syncCount, newChatOpen, railView, markAllRead, pushToast } from "../../stores.js";
  let menuOpen = false;
...
<header class="sidebar-head">
  <h1>{$t("rail_chats")}{#if $syncing}<span class="sync-tag">{$t("syncing")}{#if $syncCount > 0} · {$syncCount}{/if}</span>{/if}</h1>
