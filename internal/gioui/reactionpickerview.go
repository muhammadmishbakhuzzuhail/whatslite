...
func ReactionPickerView(gtx layout.Context, th *material.Theme, t Theme) layout.Dimensions {
	...
	return layout.Center.Layout(gtx, func(gtx layout.Context) layout.Dimensions {
		return rpCard(gtx, th, t)
	})
}

// rpCard — .rp-pop: t.Bg, radius 14, 352x400, border Line (kesan box-shadow-lg).
func rpCard(gtx layout.Context, th *material.Theme, t Theme) layout.Dimensions {
	w := gtx.Dp(352)
	h := gtx.Dp(400)
	sz := image.Pt(w, h)
	r := gtx.Dp(14)
...
func rpGrid(gtx layout.Context, th *material.Theme, t Theme, w, h int) layout.Dimensions {
	cols := 8
	pad := gtx.Dp(12)
	cell := gtx.Dp(40)
	...
	for i := 0; i < len(rpEmoji); i++ {
		...
		x := x0 + col*(cell+gap)
		rpCell(gtx, th, t, rpEmoji[i], x, y, cell)
	}
	return layout.Dimensions{Size: image.Pt(w, h)}
}

// rpCell menggambar satu glyph emoji terpusat dlm sel cellxcell pd offset (x,y).
func rpCell(gtx layout.Context, th *material.Theme, t Theme, glyph string, x, y, cell int) {
